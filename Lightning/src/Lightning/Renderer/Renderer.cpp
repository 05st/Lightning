...
namespace Lightning {
	Renderer::Renderer(bool defaultShaders) {
		if (defaultShaders) {
	...
	void Renderer::Draw() {
		for (VertexArray* vertexArray : toDraw) {
			vertexArray->Bind();
	...
	void Renderer::AddVertexArray(VertexArray* vertexArray) {
		toDraw.push_back(vertexArray);
	}
